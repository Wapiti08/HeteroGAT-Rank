...
from comp.old_models.hetergat import HeterGAT
from comp.old_models.dhetergat import DiffHeteroGAT
from comp.old_models.pnhetergat import PNHeteroGAT
...
if __name__ == "__main__":
    ...
    #     _ = model2(dummy_batch)

    # optimizer2 = torch.optim.Adam(model2.parameters(), lr=0.001, weight_decay=1e-4)

    # model2, optimizer2, train_loader, test_loader = accelerator.prepare(
    #                 model2, optimizer2, train_loader, test_loader
    #                     )
    ...
    #         # compute loss
    #         loss = accelerator.unwrap_model(model2).compute_loss(logits, batch)
    ...
    # # Compute metrics
    # metrics = accelerator.unwrap_model(model2).evaluate(all_logits, all_labels)

    # print("Evaluation Metrics for HeteroGAT model: ", metrics)

    # accelerator.unwrap_model(model2).plot_metrics(
    #     all_labels,
    ...
    with torch.no_grad():
        ...
        dummy_batch = dummy_batch.to('cpu')
    ...
    # Compute metrics
    metrics = accelerator.unwrap_model(model3).evaluate(all_logits, all_labels)

    print(f"Evaluation Metrics for {model_name} model: ", metrics)

    accelerator.unwrap_model(model3).plot_metrics(
        all_labels,
        torch.sigmoid(all_logits).cpu().numpy(),
        metrics)
