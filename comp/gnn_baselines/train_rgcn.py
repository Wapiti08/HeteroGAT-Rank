...
def main() -> None:
    ...
    # Build optional class weights (ignore y=-1).
    class_weight = None
    if args.reweight:
        ys = []
    ...
    # y == -1 means "unknown label". Ignore them in loss/metrics.
    ...
    # AUROC/AUPRC may be undefined if test has only one class.
    auroc = None
    auprc = None
    try:
    ...
    # Ranking-style hunting metrics (budgeted triage).
    ks: list[int] = []
    ...
    order = y_prob1.argsort()[::-1]  # descending
    ...
    for k in ks:
        ...
        prec_k = tp_at_k / kk
        rec_k = tp_at_k / pos_total
        print(f"P@{k}={prec_k:.4f} R@{k}={rec_k:.4f} (tp@{k}={tp_at_k}, pos_total={pos_total})")
