...
from comp.gnn_baselines.train_common import (  # noqa: E402
    class_weight_from_dataset,
    evaluate_classifier,
    parse_ks,
    read_list,
)
from model.gat import GATHomGraphClassifier  # noqa: E402


def _save_ckpt(*, path: Path, model: GATHomGraphClassifier) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    nnt = int(model.node_type_emb.num_embeddings) if model.node_type_emb is not None else 0
    nr = int(getattr(model, "_num_relations", 0))
    torch.save(
        {
            "kind": "gat_hom_graph_classifier",
            "model_kwargs": {
            ...
            "schema": {"num_node_types": nnt, "num_relations": nr},
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable per-node canonical features (type-only inputs; often collapses)",
    )
    ap.add_argument("--ks", type=str, default="5,10,20", help="Comma-separated list of K for Precision@K / Recall@K")
