...
from model.han import HANGraphClassifier  # noqa: E402


def _save_ckpt(*, path: Path, model: HANGraphClassifier) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "kind": "han_graph_classifier",
            "model_kwargs": {
...
def main() -> None:
    ...
    model = HANGraphClassifier(hidden_dim=int(args.hidden_dim), heads=int(args.heads), dropout=0.2, num_classes=2).to(device)
