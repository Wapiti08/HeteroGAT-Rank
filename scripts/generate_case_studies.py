...
def _anomaly_score(ranked: list[RankedEdge], *, k: int) -> float:
    if not ranked:
        return float("nan")
...
def _emit_case_markdown(
    *,
    ...
    lines.append(f"- **triage rank (lower is more suspicious)**: base(inv) `{base_rank}` → rarity(inv) `{rarity_rank}`")
    lines.append("")
    lines.append("#### Top-K explanation edges (base)")
    lines.append("")
    lines.append("| score | etype | src | dst |")
    lines.append("|---:|---|---|---|")
    for r in base_edges[:k_show]:
        lines.append(_format_edge_row(r))
    lines.append("")
    lines.append("#### Top-K explanation edges (rarity-adjusted)")
    lines.append("")
    lines.append("| score | etype | src | dst |")
    lines.append("|---:|---|---|---|")
...
def main() -> None:
    ...
    # Compute base + rarity(inv) ranks over all labeled graphs.
    rows = []
    for gp in graph_paths:
        ...
        ranked = topk_edges(backbone=backbone, explainer=explainer, hetero_batch=data, k=max(int(args.k) * 20, int(args.k)))
        ...
        base_anom = _anomaly_score(ranked, k=int(args.k))
        ...
        rar_anom = _anomaly_score(ranked_r, k=int(args.k))

        # inverted triage score: larger is more suspicious
        rows.append(
            {
                "path": gp,
                "y": int(y),
                "base_triage": float(-base_anom),
                "rarity_triage": float(-rar_anom),
                "base_edges": ranked[: int(args.k)],
    ...
    chosen = [i for _, _, i in cand[: int(args.topn)]]
