...
def explain_with_gradcam_vec(model, dataloader, device, target_class=1, max_batches=None):
    """
    Grad-CAM style explanation to rank edge and node importance.
    Also maps node scores back to original node values using model.reverse_node_id_map.
    ...
    reverse_node_id_vec = getattr(model, "reverse_node_id_vec", [])
    batch_count = 0
    ...
    with torch.enable_grad():
        for batch in dataloader:
            ...
            assert attn_weights, "attn_weights is empty"
            assert edge_index_map, "edge_index_map is empty"
